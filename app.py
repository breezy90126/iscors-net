...
with gr.Blocks(title='iSCORS axes viewer') as demo:
    gr.Markdown(
        ...
        '**這不是在說論文的 slope=3 有錯，是統計層級不同**：'
        '論文的 slope=3 把**每一顆完整的細胞核**當一個資料點（不同細胞/藥物處理下 V_DLS、1/D\\* '
        '各自平均成一個值），是**跨核（population）**的散佈圖統計；'
        '這裡的延伸把**同一顆核內部的每一個像素**當一個資料點，問的是**單核內、逐像素**的 '
        '1/D\\*–CV² 關係——用的是跟 PCA 同一類工具（廣義特徵分解），但統計母體完全不同。'
        '這個 repo 量到的「核內逐像素斜率 ~0.5」跟論文的「跨核斜率 3」並不矛盾，'
        '因為兩者本來就不是同一個統計層級。可複現性檢驗的結論是：單核內逐像素能穩健找到的生物軸大約只有一條'
        '（核仁/密度），細節見 `RETROSPECTIVE_session.md`。\n\n'
