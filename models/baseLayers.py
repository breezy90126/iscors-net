...
def conv2d(in_channels, out_channels, kernel_size, bias, padding = 1, groups = 1):
...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding = 1):
    ...
    for i, char in enumerate(order):
        if char == 'r':
            ...
            # print(in_channels, out_channels, min(in_channels,out_channels) // 2)
            modules.append(('conv', conv2d(in_channels, out_channels, kernel_size, bias, padding = padding,
                                           groups = min(in_channels,out_channels) // 2)))
        elif char == 'g':
...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size = 3, order = 'cbr', num_groups = 8, padding = 1):
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size = 3, order = 'cbr', num_groups = 8):
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size = 3, apply_pooling = True,
                 pool_kernel_size = (2, 2), pool_type = 'max', basic_module = DoubleConv, conv_layer_order = 'cbr',
                 num_groups = 16):
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size = 3,
                 scale_factor = (2, 2), basic_module = DoubleConv, conv_layer_order = 'cr', num_groups = 8):
    ...
    def forward(self, encoder_features, x, use_skip_connections = True):
